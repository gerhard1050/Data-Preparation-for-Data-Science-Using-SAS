/*-------------------------------------------------------------------------------------------
Version 2 - 27.3.2018 - Thanks to Jos van der Velden for his remarks:
- Formatname SO instead of SO_FMT
- All Formats need to be recreated first(existing Format Catalog was in 32-bit)
- Dataset SALES_10 does not exist
- Dataset LEAD_BASE does not exist

-------------------------------------------------------------------------------------------*/

** REMARK: Delete the formats.sas7bcat catalog in this library if it dates from 2010 or earlier. 
           It is most likely an old 32-bit catalog that cannot be re-used;
...
/*** Added 2018-03-27 - Thanks to Jos Jos van der Velden for his remark 
     sorted dataset is missing otherwise
***/
proc sort data= sales.orders out=orders10_sort;
...
/*** Added 2017-03-27 - Dataset LEAD_BASE was missing ***/
data sales.lead_base(keep= productid productgroupid price);
 set sales.ordermart(where=(shopid = 580));
 by productid;
 Price = round(AvgPrice*1.05,0.01);
